"""Convert a Song into a Spotify track ID."""
...
class SpotifyConverter(spotipy.Spotify):
    ...
    def uri_to_song(self, url: str) -> song.Song:
        ...
        if track is not None:
            self.__commit_song(track['id'],
                            track['external_ids']['isrc'],
                            track['name'],
                            track['artists'][0]['name'])

            result_song = song.Song(source="spotify",
                                uid=track['id'],
    ...
    def song_to_url(self, a_song: song.Song) -> tuple[str, str]:
        ...
        if a_song.isrc is not None:
            self.cur.execute("SELECT uid FROM spotify WHERE isrc=? limit 1", [a_song.isrc])
            track = self.cur.fetchone()
            if track is not None:
                uid = track[0],
        ...
        track = self.search(q=f'track:{a_song.title} artist:{a_song.first_artist}',
                            limit=1,
                            type='track')
        if track is not None:
            uid = track['tracks']['items'][0]['id']
